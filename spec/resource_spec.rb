...
RSpec.describe CertifyActivityLog do
  ...
  context "handles writing logs and errors" do
    ...
    let(:error_response) { CertifyActivityLog::Resource.handle_excon_error error }

    before { error.set_backtrace backtrace }

    it "raises an error if a bad log level is specified" do
      expect { CertifyActivityLog::Resource.logger.foo("bad error") }.to raise_error(NoMethodError)
    end

    it "logs the message for a valid log level" do
      expect { CertifyActivityLog::Resource.logger.error("bad error") }.not_to raise_error
    end
