...
module V2
  RSpec.describe CertifyActivityLog do
    before do
      CertifyActivityLog.configuration.activity_api_version = 2
    end
    ...
    describe "get activities operations using where" do
      context "for getting activities" do
        ...
        let(:activities) { CertifyActivityLog::Activity.where({recipient_uuid: 1}) }
      ...
      context "with bad parameters" do
        let(:activities) { CertifyActivityLog::Activity.where({application_uuid: "foo"}) }
        let(:body) { activities[:body] }
